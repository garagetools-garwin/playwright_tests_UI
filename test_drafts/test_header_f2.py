...
def test_header_link(page):
    ...
    expect(page).to_have_url(re.compile('https://garwin.ru/'))
    page.get_by_role("banner").get_by_role("link", name="Акции").click()
    page.wait_for_url(f'{url}promos') # wait url
    expect(page).to_have_url(f'{url}promos')# proverka na to chto url sootvetstvuet
    ...
    page1.wait_for_url('https://www.youtube.com/@garwin_tools')
